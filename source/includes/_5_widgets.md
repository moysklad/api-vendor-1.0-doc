...
|----|----|----|
|1000| Недопустимое состояние (виджета или попапа) state для сообщения message.name, допустимые состояния: message.expectedStates |Не допускается отправка данного сообщения из текущего состояния виджета или попапа| 
|1001|Отсутствует обязательный параметр parameter.name| В сообщении отсутствует обязательный параметр|
|1002|Некорректное значение 'value' ( [typeof](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/typeof) (value)) параметра parameter.name | Параметр сообщения имеет некорректное значение.| 
|1003|Параллельный запрос message.name|Не допускается отправка виджетом повторного запроса до момента получения ответа на предыдущий такой же запрос|
