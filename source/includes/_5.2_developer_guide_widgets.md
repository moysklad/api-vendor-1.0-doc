...
за исключением протокола [user-context](#protokol-user-context).
...
Рекомендуемый способ получения контекста — [протокол user-context](#protokol-user-context): окно или виджет запрашивает
у хост-окна одноразовый токен и обменивает его на сервере разработчика через
...
По умолчанию хост-окно добавляет в качестве параметров запроса следующие данные: 
...
* [Протокол user-context](#protokol-user-context).
...
#### Протокол user-context

> Дескриптор решения, у которого главное окно использует протокол user-context
...
> Дескриптор решения с виджетом, использующим протокол user-context
...
<ServerApplication xmlns="https://apps-api.moysklad.ru/xml/ns/appstore/app/v2"
                   ...
                   xsi:schemaLocation="https://apps-api.moysklad.ru/xml/ns/appstore/app/v2
                    https://apps-api.moysklad.ru/xml/ns/appstore/app/v2/application-v2.xsd">
  <iframes>
    <iframe type="main" sourceUrl="https://example.com/iframe.html"/>
  </iframes>
  <vendorApi>
    <endpointBase>https://example.com/dummy-app</endpointBase>
  </vendorApi>
...
Чтобы виджет, главное окно или модальное окно начали поддерживать протокол user-context, в дескрипторе необходимо
добавить в блок `uses` для `widgets`, `iframe` или `popup` тег: `<user-context/>`. Примеры смотрите в правой части
экрана. Если тег не указан, в ответ на запрос токена хост-окно вернет сообщение `InvalidMessageError` с кодом
ошибки 1004.

Атрибут `useContextKey="false"` отключает передачу устаревшего параметра `contextKey` в URL загрузки. Подробнее
