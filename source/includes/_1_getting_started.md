...
7. Если для взаимодействия с пользователем нужны кнопки в интерфейсе МоегоСклада, создайте кастомные кнопки в решении. Для этого нужно добавить в [дескриптор](#deskriptor-resheniq) тэг `buttons` и реализовать [эндпоинт обработчика нажатия на кнопку в vendorApi](#obrabotka-nazhatiq-na-kastomnuu-knopku). Подробнее читайте в разделе [Кастомные кнопки](#kastomnye-knopki).
