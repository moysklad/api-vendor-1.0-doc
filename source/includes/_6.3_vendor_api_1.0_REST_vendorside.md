...
+ **404 Not Found** — устаревший аналог для статуса **204**. **Не рекомендуется к использованию**: поддержка на стороне МоегоСклада будет прекращена в феврале 2026 г.
+ прочие статусы **4хх** обрабатываются как ошибка — переход в состояние **DeactivationFailed**.
