...
При навигации из попапа в текущей вкладке (`target` имеет значение `self`) произойдет переход, но попап будет поверх страницы. Если необходимо, что после перехода попап закрывался можно использовать сообщение `ClosePopup` подробнее в разделе [Кастомные попапы](#kastomnye-popapy-modal-nye-okna).

### Кастомные попапы (модальные окна)

> Дескриптор с виджетом и iframe, использующие кастомные попапы
...
Использование модальных окон позволяет виджетам (которые имеют фиксированную высоту и ширину) и главному iframe
