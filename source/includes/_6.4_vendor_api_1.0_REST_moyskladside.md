...
* **appUid** — appUid приложения. Может быть использован для определения того, какое приложение сейчас отображается у пользователя, если на одном и том же sourceUrl у вас находятся несколько приложений (например _dev_ и _prod_-версия).
* **appId** — ИД приложения. Может быть использован для определения того, какое приложение сейчас отображается у пользователя.


Пример загружаемого URL для приложения Онлайн-заказ (при условии, что `iframe.sourceUrl` в его [дескрипторе](#deskriptor-prilozheniq) имеет значение `https://yoursite.ru/moysklad`):
