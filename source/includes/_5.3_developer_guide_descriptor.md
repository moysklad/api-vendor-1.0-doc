...
- Чтобы на страницах списков отключить проверку наличия выбранных элементов и пропустить диалог подтверждения массовой операции, используйте атрибут `useSelected` (опциональный, по умолчанию `true`). При `false` поле выбранные элементы не учитываются.
