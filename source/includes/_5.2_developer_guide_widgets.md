...
Для этого запросите данные пользователя через эндпоинт [получения контекста пользователя](#poluchenie-kontexta-pol-zowatelq), указав полученный в URL параметр `contextKey`.
...
контекст пользователя через [Vendor API](#poluchenie-kontexta-pol-zowatelq).
...
Для упрощения работы с `postMessage` можно использовать **JS Widget SDK**. SDK дает единый API для запросов (request/response) и событий хоста, скрывая работу с `messageId`/`correlationId` и обработку ошибок `InvalidMessageError`.

Ссылка на репозиторий SDK:

- GitHub: [https://github.com/moysklad/js-widget-sdk](https://github.com/moysklad/js-widget-sdk)

Подробности по API и актуальные примеры смотрите в README репозитория.
...
- c фиксацией мажорной версии: `https://cdn.jsdelivr.net/npm/@moysklad-official/js-widget-sdk@1/dist/widget.min.js`
- c фиксацией конкретной версии: `https://cdn.jsdelivr.net/npm/@moysklad-official/js-widget-sdk@1.0.0/dist/widget.min.js`
...
* Может получать текущий контекст пользователя, так же как виджеты и главное окно решения.
