...
+ активировать/деактивировать приложение у себя в системе при установке и удалении приложения пользователем на аккаунте
+ получать токен для доступа к JSON API

то в этом случае со вендору со своей стороны реализовать REST-endpoint с доступом по HTTPS
...
соответствии со своей внутренней логикой (например, при работе механизма Retry).
...
не требует доступа к АПИ, то данный объект не придёт.
    ...
    + **scope** `Array` - какие права предоставлены на доступ данному ресурсу. Сейчас досутп предоставляется с 
     равами администратора аккаунта (`"scope": ["admin"]`).
