...
Для платных приложений открывается возможность добавить приложению [Пробный период](#probnyj-period-platnyh-prilozhenij). 
...
После того, как приложение было создано и протестировано, может потребоваться внести в него правки.
...
- Доступны просмотр и перегенерация **[Секретного ключа](#sekretnyj-kluch-secretkey)**. Для просмотра следует нажать на кнопку-глаз у скрытого звездочками 
поля, для генерации нового ключа - кнопку **Сгенерировать**.
...
Отладку приложений для Маркетплейса производится на аккаунтах разработчика. Для того, чтобы аккаунт в МоемСкладе стал 
аккаунтом разработчика, его нужно **привязать к вендору**, и это тоже можно сделать в личном кабинете.
