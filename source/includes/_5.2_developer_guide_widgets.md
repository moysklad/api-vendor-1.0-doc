...
Особенности работы документа Заказ кодов маркировки после отправки заказа кодов в Честный Знак (через кнопку Заказ кодов):

- документ доступен для редактирования, но сохранение изменений заблокировано, следует учитывать это при использовании протоколов;
- на любые изменения в документе (статусы, поля) в СУЗ будут приходить сообщения Change без последующего Save.
