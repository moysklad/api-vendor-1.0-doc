...
+ **access** `Array`— доступы к ресурсам, указанным в [дескрипторе приложения](#blok-access). Сейчас из ресурсов для приложений доступно 
    только [JSON API 1.2](https://online.moysklad.ru/api/remap/1.2). Если ваше приложение не требует доступа к API, то данный объект не придет. Не включается в тело для (`"Cause": "TariffChanged"`). Атрибуты:
