...
В случае, если виджет использует протокол **open-feedback**, система не отображает 
 ...
 При сохранении страницы с виджетом (при явном нажатии на кнопку сохранить, 
 закрытии экрана, сохранении без изменений, сохранение объекта через диалог подтверждения, 
 при листании кнопками-стрелочками на соседние объекты), если виджет на экране редактирования сущности 
 использует протокол **save-handler**, он оповещается о факте сохранения объекта пользователем, 
 получая сообщение `Save`.
 
 Использование виджетом протоколов **open-feedback**, **save-handler** необходимо указать в [дескрипторе](#deskriptor-prilozheniq) 
 приложения.
...
В случае, если виджет использует протокол **save-handler**, виджет на экране редактирования 
сущности оповещается о факте её сохранения пользователем, получая сообщение `Save`. 
 ...
 При сохранении пользователем редактируемого объекта (явном нажатии на кнопку сохранить, 
 закрытии экрана, сохранении без изменений и т.д.) хост-окно передает в iframe на странице редактирования
 сообщение `Save` через [postMessage](https://developer.mozilla.org/en-US/docs/Web/API/Window/postMessage).
