...
Для этого в дескрипторе для виджета нужно объявить поддержку опционального протокола **change-handler**.
 ...
 Узнать, для каких точек поддерживается протокол **change-handler**, можно [тут](#dostupnye-protokoly-w-bloke-widgets-supports).
...
Протокол **update-provider** поддерживается на данный момент только для точки `document.customerorder.edit`.
...
* При изменении значения на то же самое, поля на UI не обновляются (верно только для основных и дополнительных полей и не относится к позициям)
* При изменении всех значений полей на те же самые документ не становится измененным - т.е. пользователь может закрыть экран редактирования документа без диалога с вопросом “Данные были изменены. Сохранить изменения?”. К позициям это не относится, если позиции в запросе есть - список всегда обновляется, документ считается измененным.
...
* Для одновременного изменения согласованных полей, таких как Организация (Контрагент), Счет и Договор необходимо чтобы их значения были совместимы: счет должен принадлежать указанной организации, договор должен относиться к этой организации и контрагенту .
