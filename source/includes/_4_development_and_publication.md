...
Краткая инструкция по разработке, отладке и публикации приложений на примере серверных приложений.

### Разработка серверных приложений

Серверное приложение — это приложение для взаимодействия с МоимСкладом по JSON API. Для аутентификации JSON API использует access-токен. Чтобы его получить, при разработке приложения необходимо реализовать эндпоинт согласно спецификации [Vendor API 1.0](https://dev.moysklad.ru/doc/api/vendor/1.0/#vendor-api-1-0).

Доступ к данным пользователей осуществляется по [JSON API 1.2](https://dev.moysklad.ru/doc/api/remap/1.2/#mojsklad-json-api).
...
* Будет ли в iframe-части серверного приложения какая-либо пользовательская функциональность, кроме настроек? Если да - то,
 возможно, следует предусмотреть, чтобы пользовательская функциональность была доступна не только администратору аккаунта 
...
Приложение может быть создано на любом языке программирования, с использованием любых фреймворков. 

Документация по JSON API 1.2 для доступа к данным пользователей: <a href="https://dev.moysklad.ru/doc/api/remap/1.2/" target="_blank">https://dev.moysklad.ru/doc/api/remap/1.2</a>.

Короткое <a href="https://www.youtube.com/watch?v=eQWNADSRSWA" target="_blank">видео</a> о том, как работать с API МоегоСклада.
...
При создании Серверного приложения можно указать [платным или бесплатным](#platnye-i-besplatnye-prilozheniq)
...
* Использование основной iframe-части для настройки приложения администратором аккаунта с обновлением статуса в Маркетплейсе.
