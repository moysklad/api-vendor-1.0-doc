...
Ведутся работы по добавлению протокола `change-handler` в основные документы. 
Сейчас протокол доступен в:
- Заказ покупателя
- Отгрузка
...
В случае, если виджет поддерживает протокол **open-feedback**, система не отображает 
 ...
 содержащее несохраненное состояние документа, за исключением некоторых ещё не поддерживаемых полей, например:
- сost (себестоимость)
...
Для этого в дескрипторе для виджета нужно объявить поддержку опционального протокола **change-handler**.
 ...
 - при изменении некоторых пока не поддерживаемых полей, например себестоимость и накладные расходы (cost, overhead)  
 ...
 Протокол **change-handler** поддерживается для точек: 
