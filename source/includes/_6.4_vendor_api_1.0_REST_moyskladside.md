...
+ [Получение контекста пользователя](#poluchenie-kontexta-pol-zowatelq)
...
Для получения контекста в URL, по которому загружается окно, модальное окно или виджет, добавляются следующие параметры:
...
* **userLocale** — язык пользователя. Используется, если ваш _iframe_ имеет перевод на другие языки. Возможные значения `ru_RU`, `en_US`.
...
`https://yoursite.ru/moysklad?contextKey=1c14e98cd272239c03bf3d9697f167699743292c&appUid=online-order.moysklad&appId=f0e50ffd-e267-46bf-a963-0adcf2fe09e0&userLocale=ru_RU`.
