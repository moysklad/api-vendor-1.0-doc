...
Также серверное приложение может иметь свою вкладку среди iframe-приложений и пользовательский интерфейс. Используя 
 ...
 МоегоСклада / администратора аккаунта. 
...
Iframe-приложение дает возможность быстро разработать и разместить простое приложение на площадке МоегоСклада. Однако такие 
приложения тоже должны соответствовать [требованиям к приложениям](https://moysklad.gitpages.moysklad.ru/remap-deployer/vendor_10/AS-343/#trebowaniq-k-prilozheniqm).

Для iframe-приложений поддерживается доступ к контексту пользователя, однако **не поддерживается** активация и 
деактивация по Vendor API, а также доступ по токену к JSON API 1.2.
