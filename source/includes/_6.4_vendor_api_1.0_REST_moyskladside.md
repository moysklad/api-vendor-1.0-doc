...
Базовый URL REST-эндпоинтов со стороны МоегоСклада (далее — `ENDPOINT`):
...
**Resource**: `ENDPOINT/apps/{appId}/{accountId}/status`
...
**Resource**: `ENDPOINT/apps/{appId}/{accountId}/loyalty`
...
**contextKey** — это временный ключ, который может быть использован в течение 5 минут с момента загрузки iframe для получения контекста пользователя через данный эндпоинт. Если был использован **contextKey**, после окончания времени его
жизни эндпоинт вернет ошибку `404 Not Found`.

**Resource**: `ENDPOINT/context/{contextKey}`
