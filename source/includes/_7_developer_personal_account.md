...
##### Рекомендуемый порядок тестирования и публикации изменений

Для тестирования и безопасной публикации функциональных изменений в опубликованном решении рекомендуется следующий порядок действий:

1. Создайте версию для редактирования (как описано выше). Для начальной разработки также можно создать отдельное решение с собственным псевдонимом (например, `myapp_dev`).
2. В дескрипторе версии (или отдельного решения) укажите URL вашего тестового сервера: в теге [vendorApi/endpointBase](#blok-vendorapi) — для серверной части (например, `https://my-test-server.com/app`), а также в тегах `sourceUrl` блоков `iframes`, `widgets` и `popups` — для UI-части, если она присутствует в решении.
3. Протестируйте изменения на аккаунте разработчика. На этом этапе опубликованное решение продолжает работать для пользователей без изменений.
4. После завершения тестирования разверните новую версию кода на продакшн-сервере (например, `https://my-prod-server.com/app`).
5. В дескрипторе версии замените URL тестового сервера на URL продакшн-сервера.
6. Отправьте версию на модерацию.

Модератор проверит решение на продакшн-сервере, используя временные учетные данные версии (appId, appUid и секретный ключ с вкладки **Учетные данные решения**). Пользователи при этом продолжают работать с учетными данными, назначенными при первой публикации решения. Разработчик может отличить запросы модератора от запросов пользователей по набору учетных данных (appId/appUid/secretKey) во входящих запросах.

Таким образом, модератор всегда проверяет именно тот код, который после нажатия кнопки **Опубликовать** станет доступен конечным пользователям.

##### Обратная совместимость

При обновлении решения продакшн-сервер должен корректно обрабатывать запросы как от текущей (опубликованной) версии решения, так и от новой версии. В момент модерации сервер уже обслуживает оба набора учетных данных — опубликованного решения и версии. Обеспечьте обратную совместимость на стороне сервера.

##### Когда полный процесс не требуется

Если изменения затрагивают только контентную часть решения (описание, скриншоты, инструкцию, названия и описания тарифов) и не касаются дескриптора или серверной логики, полный цикл с переключением URL не нужен. Достаточно создать версию с контентными изменениями и обеспечить её успешную установку для прохождения модерации.

##### Комментарий при отправке на модерацию

При отправке версии на модерацию рекомендуется оставить комментарий с описанием внесенных изменений. Если были сделаны функциональные изменения (затронут дескриптор или серверная логика), укажите также учетные данные для тестирования и, при необходимости, особые инструкции для проверки. В окне отправки на модерацию есть напоминание об этом.
...
Редактирование контактной информации разработчика (электронная почта, Telegram, WhatsApp, Max) доступно всегда.
...
Отладить неопубликованное решение можно в аккаунте разработчика. Неопубликованные решения разработчика находятся в каталоге решений в блоке **Решения в разработке**. 

Для отладки установите решение на свой аккаунт разработчика. Длительность подписки составит 24 часа с момента установки. Подписку можно продлевать ежедневно и без ограничений.

Неопубликованные решения видит только разработчик, для пользователей они не видны. Такие решения можно подключать и отключать так же, как и обычные опубликованные решения. 
Ограничение в 24 часа распространяется только на неопубликованные решения. Подписки на публичные и приватные решения действуют согласно выбранному и оплаченному периоду.
...
При открытии инструкции в каталоге МоегоСклада изображение будет показано в модальном окне. При открытии инструкции по прямой ссылке изображение откроется в новой вкладке.

Рекомендуется всегда заполнять атрибут `alt`, чтобы описание изображения было доступно пользователям и могло использоваться в интерфейсе предпросмотра.

#### Видео в инструкции

Для демонстрации функциональности решений в инструкции можно встраивать видео двумя способами:

* через тег `iframe` — если видео загружено на сторонний видеохостинг, например VK, RuTube или YouTube;
* через тег `video` — если используется прямая ссылка на видео, размещенное на собственном сервере.

Атрибут `data-manual-zoom` применяется только к изображениям. Для видео он не работает: видео не открывается в увеличенном виде или в модальном окне.
...
* полноэкранный просмотр видео через атрибут `allowfullscreen` не работает в инструкции;
* оптимальный размер видео — 640 × 360 px;
* максимальная ширина контента в инструкции — 640 px;
* при встраивании видео по прямой ссылке предпочтительно использовать формат mp4, так как он поддерживается большинством современных браузеров.
...
2. Разрешение изображений: от 1280x720 до 1920x1080 пикселей, соотношение сторон 16:9.
3. Размер каждого файла - не более 2 Мб.
...
| tariffId  | ID тарифа                                                                                                              | `install_app`                                                             |
| place     | Место вызова события:<br>- `blog` — из статьи<br>- `header` — из верхней кнопки<br>- `tariff_grid` — из тарифной сетки | `open_app`, `install_app`                                                 |
| sourceId  | ID статьи, только для `place=blog`                                                                                     | `open_app`, `install_app`                                                 |
| appStatus | Статус решения                                                                                                         | `go_to_app`                                                               |
