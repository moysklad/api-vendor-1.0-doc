...
Чтобы виджет начал поддерживать попап-окна необходимо добавить блок 
```
<popups>
    ...
</popups>
```
в дескрипторе приложения. Пример дескриптора с блоком `popups` можно увидеть справа.
...
В этом случае пользователь конечно так же может закрыть попап принудительно, при этом параметры переданы в виджет не будут.

Пример попап-окна с кнопками Закрыть и Сохранить.
...
* передача в виде объекта (объект при этом может быть пустым - `null`)
...
* Попап-окно является модальным, т.е. открываются поверх текущей страницы МС и требуют действия от пользователя внутри этого окна (взаимодействие с веб-страницей и/или закрытие попапа).
...
* Попап также, как и прочие iframe-приложения, может получать текущий контекст пользователя.
