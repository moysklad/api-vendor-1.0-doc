...
| Запуск пробного периода решения  | JavaScript-событие | `start_trial`      |
...
Для каждого события МойСклад может заполнять следующие дополнительные параметры:

| Параметр  | Описание                                               | Для каких событий                        |
|-----------|--------------------------------------------------------|------------------------------------------|
| appId     | ИД решения                                             | все события                              |
| accountId | ИД аккаунта                                            | все события на внутренней витрине        |
| tariffId  | ИД тарифа                                              | `install_app`, `start_trial`             |
| place     | Место вызова события (`blog`, `header`, `tariff_grid`) | `open_app`, `install_app`, `start_trial` |
| sourceId  | ИД статьи, только когда `place=blog`                   | `open_app`, `start_trial`                |
