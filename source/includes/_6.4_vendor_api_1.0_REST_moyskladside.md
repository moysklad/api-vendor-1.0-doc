...
+ [Получение контекста пользователя](#poluchenie-kontexta-pol-zowatelq) (устаревший способ)
...
Если разработчик ранее уже ответил МоемуСкладу статусом **Activated**, то вызывать данный эндпоинт не следует.
...
Виджет запрашивает у хост-окна одноразовый токен по [протоколу user-context](#protokol-user-context) и передает его
...
|:---------|:-------|:-----------------------------------------------------|:-------------|
| token    | String | Одноразовый токен из сообщения `UserContextResponse` | Да           |

<u>Тело ответа</u>:

| Название  | Тип    | Описание                                                                          | Обязательное |
|:----------|:-------|:----------------------------------------------------------------------------------|:-------------|
| accountId | UUID   | Идентификатор аккаунта МоегоСклада                                                | Да           |
| userId    | UUID   | Идентификатор пользователя                                                        | Да           |
| userUid   | String | Логин пользователя (uid)                                                          | Да           |
...
1. Передавайте токен только в теле POST-запроса. Передача токена в URL запроса или в заголовках не поддерживается.
1. Выполняйте запрос только с сервера разработчика: секретный ключ решения, которым подписывается JWT-токен, не должен
   попадать в браузер пользователя.
1. Не сохраняйте токен в `localStorage`, `sessionStorage` и cookie и не записывайте его в логи.
...
##### Переход с получения контекста по contextKey

Получение контекста по `contextKey` продолжает работать параллельно с новым способом. Чтобы перейти на получение
контекста по одноразовому токену:
...
1. Запрашивайте токен у хост-окна по [протоколу user-context](#protokol-user-context).
...
Способ является устаревшим: в новых решениях используйте
[получение контекста пользователя по одноразовому токену](#poluchenie-kontexta-pol-zowatelq-po-odnorazowomu-tokenu).
