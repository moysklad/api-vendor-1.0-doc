...
Для обработки установки и удаления решения (приостановки и возобновления) на аккаунте пользователя МоегоСклада требуется реализовать на сервере разработчика эндпоинт имеющий доступ по **HTTPS** с адресом: 
...
```json
{
  ...
  "user": {
    "userId": "23ca69d4-2657-40c4-8ba1-6ce24ddeac2e", 
    "role": "admin"
  }
}
```
...
+ **objectId** `UUID` — идентификатор документа, в котором кнопка была нажата. Не заполняется для точек встраивания `*.create`;
+ **user** — объект, описывающий данные текущего пользователя. Поля:
  + **userId** `UUID` — идентификатор пользователя (employee), который нажал кнопку;
  + **role** `String` — название роли пользователя. Возможные значения: `admin`, `cashier`, `worker`, `individual`. 
...
Если решению для обработки действия требуется дополнительная информация о сотруднике, например информация о правах, то ее необходимо запросить через эндпоинт    
[информации о сотруднике в JSON API](https://dev.moysklad.ru/doc/api/remap/1.2/dictionaries/#suschnosti-sotrudnik-poluchit-sotrudnika).
