...
### Просмотр логов ваших приложений

Вы можете просматривать логи запросов к решениям через интерфейс кабинета разработчика. Логи содержат информацию об основных взаимодействиях по vendor API:
...
Логи отображают исходящие от нас запросы (OUTGOING REQUEST) и полученный от вас ответ (RESPONSE of OUTGOING REQUEST).

#### Как получить доступ к логам

Личный кабинет разработчика -> Решения -> Ваше серверное решение -> Вкладка 'Логи'

#### Примечания

* Токены авторизации и access_token скрываются по соображениям безопасности
* Мы храним логи для кабинета разработчика 14 дней.
* Более старые логи можно запросить через техподдержку (не старше 30 дней)
