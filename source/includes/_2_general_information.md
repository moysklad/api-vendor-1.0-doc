...
Вендоры могут указать гибкие права доступа для приложений, т.е. выбирать в явном виде только те пермиссии (permissions), которые им необходимо
получить от юзера для пользования их приложением. Для этого необходимо в дескрипторе приложения выбрать scope **custom** и в подразделе **permissions**
указать все необходимые уровни доступа.

Это всё необходимо для снижения уровня рисков у пользователей и увеличения количества установок приложений 
(т.к. у некоторых пользователей есть опасения устанавливать приложения с широким набором прав).

Сам же блок **permissions** может включать в себя два вида прав:

**1. Права сущностей** — если указан данный тип, то он обязательно должен включать в себя
необходимые уровни доступа к приложению (view, create, update, delete, print, approve).

**2. Пользовательские права** — для данного типа достаточно лишь указать название права, дополнительной информации
по ней не требуется.

Более подробную информацию можно найти в [Информация о правах доступа](#blok-access).
