...
При получении сообщения от виджета хост-окно производит валидацию сообщения и, в случае если проверка не пройдена, возвращает в ответ сообщение `InvalidMessageError` с описанием ошибки.
...
|1002| Некорректное значение '%value%' (%typeof(value)%) параметра %parameter.name% | Параметр сообщения имеет некорректное значение или тип. Здесь typeof - [существующий оператор в JavaScript](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/typeof) | `Некорректное значение '1' (string) параметра openMessageId`
|1002| %Некоторое произвольное сообщение% | Параметр сообщения не прошел дополнительную проверку | `В диалоге должно быть не менее одной кнопки` | 
