...
* **Специальные** — права доступа, которые используются только для приложений.  
...
- **good-folder-selector** позволяет iframe-частям приложений переиспользовать существующий в МоемСкладе селектор группы товаров с
...
<ServerApplication xmlns="https://apps-api.moysklad.ru/xml/ns/appstore/app/v2"
  ...
  <access>
    ...
    <permissions>
      <useOwnWebhooks/>
      <!-- Используйте <useAllWebhooks/> если приложение должно управлять вебхуками всех приложений -->
    </permissions>
  </access>
</ServerApplication>
```
