...
* **Пользовательские** — права доступа, в которых достаточно указать только название. Позволяют получить доступ к
  ...
  `<viewProfitAndLoss/>`, `<viewMoneyDashboard/>`, `<viewProductCostAndProfit/>`, `<viewCustomerBalanceList/>`, `<viewStockReport/>`.
...
- **OPERATION (view, create, update, delete, print, approve)** — purchaseOrder, invoiceIn, supply, purchaseReturn,
  ...
  processing, processingOrder, productionTask.

- **DICTIONARY (view, create, update, delete, print)** — good, inventory, company, contract, retailShift, productionStageCompletion, counterpartyAdjustment.

- **BASE (view, create, update, delete, print)** — retailStore, processingPlan, myCompany, employee, warehouse,
  currency, project, country, uom, customEntity, processingStage, processingProcess, salesChannel, taxRate.
