...
2. Маркетплейс аннулирует доступ по токену (если таковой был выдан) и выполняет HTTP DELETE запрос (для платных 
приложений в нем передается причина деактивации) к серверу вендора. То есть на момент получения вендором оповещения по деактивации 
