...
Тег **uses** - опциональный. Предназначен для сервисных протоколов, поддерживаемых виджетом. На данный момент в нем можно указать 
только протокол **uses/good-folder-selector**. Он позволяет виджетам приложений переиспользовать существующий в МоемСкладе 
селектор группы товаров с получением виджетом результата выбора пользователя. 
