...
+ **appUid** `String` устанавливаемого приложения. Может быть полезно видеть не только UUID приложения, но и его *appUid*. Например, для разбора непонятных ситуаций и/или логирования. appUid состоит из _алиас_приложения.алиас_вендора_, например **egais-integration.moysklad**
...
+ **access** `Array`— доступы к ресурсам, указанным в [дескрипторе приложения](#blok-access). Сейчас из ресурсов для приложений доступно 
    только [JSON API 1.2](https://api.moysklad.ru/api/remap/1.2). Если ваше приложение не требует доступа к API, то данный объект не придет. Не включается в тело для (`"Cause": "TariffChanged"`). Атрибуты:
...
+ **subscription** — объект, описывающий параметры текущей подписки. Поля:
    ...
    + **notForResale** `Boolean` — признак того, что аккаунт, установивший приложение, является аккаунтом партнера МоегоСклада.
