...
+ [Подписка на событие изменения прав установки](#podpiska-na-sobytie-izmeneniq-praw-ustanowki)
...
+ **objectId** `UUID` — идентификатор сущности или документа, в котором кнопка была нажата. Не заполняется для точек встраивания `*.create, *.list`;
+ **selected** `Object[]` — список сущностей или документов в списке, для которых кнопка была нажата. Не заполняется для точек встраивания `*.create, *.edit`. Поля:
    + **id** `UUID` — идентификатор сущности или документа;
...
#### Подписка на событие изменения прав установки

Вы можете получать обновлённые права установки от МоегоСклада в случаях, когда права для токена у пользователя изменены.  
...
1. Объявить о поддержке события `updatePermissions` через дескриптор, добавив в `vendorApi` блок `additionalEvents` с событием `updatePermissions`
2. Реализовать на сервере разработчика эндпоинт с адресом  
...
> Пример дескриптора с событием обновления прав
