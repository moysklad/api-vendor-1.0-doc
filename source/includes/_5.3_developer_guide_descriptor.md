...
* **Специальные** — права доступа, которые используются только для решений.  
  В настоящий момент указываются для работы с вебхуками и могут принимать следующие значения: `<useOwnWebhooks/>` и `<useAllWebhooks/>`.
  [Подробнее про работу с вебхуками](#rabota-s-webhukami). 
...
> Блок buttons с кнопками в Заказе покупателя
...
Служит для задания списка кастомных кнопок, которые будут появляться в документах МоегоСклада.

- Чтобы задать имя кнопки, отправляемое на сервер при нажатии, используйте атрибут `button.name` (обязательный).
- Чтобы задать заголовок кнопки, отображамый в меню, используйте атрибут `button.title` (обязательный).
...
> Дескриптор для серверных решений с виджетом в карточке контрагента, Заказе покупателя и Отгрузке и протоколами
...
> Дескриптор для серверных решений с виджетом в Заказе покупателя и протоколом change-handler c validation-feedback
...
> Дескриптор для серверных решений с двумя кнопками на странице Заказа покупателя
