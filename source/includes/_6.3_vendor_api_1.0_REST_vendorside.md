...
+ **additional** — опциональный объект, описывающий дополнительные параметры, необходимые для работы решения. Поля:
    ...
    + **qrPayApi** — объект с параметрами для работы с QrPayAPI. Заполняется только для решений с блоком `qrPayApi` в [дескрипторе](#blok-qrpayapi);
      + **id** `UUID` — идентификатор регистрации установки на аккаунт;
      + **token** `String` — токен доступа, которым будут подписаны запросы от МоегоСклада к вендору.
...
``` 
> ---
> Пример запроса при установке решения c QrPayAPI на аккаунт:

> **Request:**

> **PUT**
> https://example.com/dummy-app/api/moysklad/vendor/1.0/apps/5f3c5489-6a17-48b7-9fe5-b2000eb807fe/f088b0a7-9490-4a57-b804-393163e7681d
...
```json
{
  ...
  "additional": {
    "qrPayApi": {
      "id": "23ca69d4-2657-40c4-8ba1-6ce24ddeac3c",
      "token": "fdsfsgrgrgershtrthrtvsfvdfvswe"
    }
  }
}
```
