...
- активировать или деактивировать решение у себя в системе при установке, приостановке, возобновлении или удалении решения на аккаунте пользователя МоегоСклада; 
...
- *example.com/dummy-app* — URL, указанный в [дескрипторе решения](#deskriptor-resheniq);
...
+ **appUid** `String` решения. Может быть полезно видеть не только UUID решения, но и его *appUid*. Например, для разбора непонятных ситуаций и/или логирования. appUid состоит из _алиас_решения.алиас_вендора_, например **egais-integration.moysklad**
...
+ **access** `Array`— доступы к ресурсам, указанным в [дескрипторе решения](#blok-access). Сейчас из ресурсов для решений доступно 
    только [JSON API 1.2](https://api.moysklad.ru/api/remap/1.2). Если ваше решение не требует доступа к API, то данный объект не придет. Не включается в тело для (`"Cause": "TariffChanged" | "Autoprolongation"`). Атрибуты:
...
| Статус               | Описание                                                           | Дальнейшие действия разработчика                                                                                                          | Отображаемый пользователю статус решения на витрине решений |
|----------------------|--------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------|
