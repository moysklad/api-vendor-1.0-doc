...
За настройку и работу окон решения отвечает [блок iframes в дескрипторе](#blok-iframes).
В случае отсутствия этого блока считается, что у решения отсутствуют окна.

В настоящий момент поддерживается 3 типа окон:

* главное окно
* окно чатов
* мобильное окно

Об ограничениях содержимого окон читайте в разделе [Ограничения для контента, загружаемого в виджетах](#ogranicheniq-dlq-kontenta-zagruzhaemogo-w-widzhetah).

При необходимости используйте [контекст пользователя](#kontext-pol-zowatelq) для того, чтобы разграничить доступ к функциям решения для администраторов и пользователей аккаунта.
...
Главное окно решения — это основной iframe для настройки или работы с серверным решением, который открывается при нажатии 
...
Если содержимое iframe не умещается в минимальную допустимую высоту окна (768px), то необходимо настроить автоматическое 
масштабирование высоты окна в зависимости от контента. Для этого в iframe нужно реализовать отправку сообщения
`EventMessage` при любом изменении высоты контента:
...
Более подробный пример использования главного iframe можно увидеть в [демо-решениях](#demo-resheniq).

#### Окно чатов

Позволяет встроить отдельный iframe решения в [Чаты](https://support.moysklad.ru/hc/ru/articles/203325403-Лента-событий#h_01K8X50ZCSN54FXVYQFCQ87R9D).

Пример чатов с несколькими решениями разработчиков:

![useful image](chats.png)

Окно чатов начинает отображаться только после перехода решения в статус **Activated**.
Если решение предполагает настройку и статус **SettingsRequired**, окно отображается после настройки решения.
...
3. Не поддерживаются [сервисы хост-окна](#serwisy-host-okna), в том числе
   [протокол контекста пользователя](#protokol-kontexta-pol-zowatelq).

#### Мобильное окно

Позволяет встроить отдельный iframe решения в мобильное приложение МоегоСклада.

Мобильное окно начинает отображаться только после перехода решения в статус **Activated**.
Если решение предполагает настройку и статус **SettingsRequired**, окно отображается после настройки решения.

Отличия в реализации мобильных окон от главных iframes:

1. Автоматическое масштабирование высоты окна не поддерживается (`expand = false`)
2. Не поддерживаются [сервисы хост-окна](#serwisy-host-okna), в том числе
   [протокол контекста пользователя](#protokol-kontexta-pol-zowatelq)
3. Содержимое окна должно быть адаптировано под мобильный экран и не должно создавать горизонтальную прокрутку
...
с передачей текущего [контекста пользователя](#kontext-pol-zowatelq).
...
Виджет можно скрыть, указав высоту `0px`. Скрытые виджеты не отображаются на страницах, но могут использовать все возможные протоколы, например [протокол **validation-feedback**](#validaciq-sostoqniq-redaktiruemogo-ob-ekta). 
...
Если виджет поддерживает [протокол **validation-feedback**](#validaciq-sostoqniq-redaktiruemogo-ob-ekta), то в ответ на сообщение `Change` он может запрещать хост-окну сохранять документ, если тот невалиден.
...
за исключением [протокола контекста пользователя](#protokol-kontexta-pol-zowatelq).
...
После нажатия кнопки «Сохранить» хост-окно не более 3 секунд ожидает `ValidationFeedback` на последнее сообщение `Change`. Полученный ответ применяется, а при его отсутствии сохранение блокируется и пользователь видит сообщение с названием виджета и адресом электронной почты поддержки вендора. Рекомендуется отвечать на `Change` синхронно, а тяжелые проверки выполнять асинхронно и затем уточнять вердикт.
...
Ошибки, характерные для протокола **update-provider** (полный перечень см. в разделе [Ошибки при работе с виджетами](#oshibki-pri-rabote-s-widzhetami)):
...
* Может получать текущий [контекст пользователя](#kontext-pol-zowatelq), так же как виджеты и главное окно решения.
...
При первом открытии страницы с окном или виджетом (в рамках одной вкладки браузера) МойСклад загружает его содержимое GET-запросом по адресу, указанному в теге sourceUrl. 
При этом хост-окно добавляет в качестве параметров запроса следующие данные: 

* **contextKey** — временный ключ, который может быть использован для получения данных пользователя через [Vendor API](#poluchenie-kontexta-pol-zowatelq).
  Время жизни этого ключа: не более 5 минут с момента генерации. 
...
Для уже загруженного и взятого из кэша iframe механизма обновления `contextKey` нет: при повторном открытии он получает только сообщение `Open`.

Для того чтобы запрашивать данные пользователя независимо от загрузки страницы рекомендуем использовать новый способ получения контекста — [протокол контекста пользователя](#protokol-kontexta-pol-zowatelq):
окно или виджет запрашивает у хост-окна одноразовый токен и обменивает его на сервере разработчика через
[POST /context/user](#poluchenie-kontexta-pol-zowatelq-po-odnorazowomu-tokenu).
Чтобы система не передавала `contextKey` в URL, укажите в дескрипторе `useContextKey="false"`.

Чтобы перейти на получение контекста по одноразовому токену:

1. Добавьте в дескрипторе решения тег `<user-context/>` в блок `uses` главного окна, виджета или модального окна.
1. Укажите атрибут `useContextKey="false"`, чтобы система перестала передавать `contextKey` в URL загрузки.
1. Запрашивайте токен у хост-окна по [протоколу контекста пользователя](#protokol-kontexta-pol-zowatelq).
1. Обменивайте токен запросом `POST /context/user` вместо `POST /context/{contextKey}`.

По умолчанию атрибут `useContextKey` имеет значение `true`, поэтому решения, в дескрипторе которых ничего не меняли,
продолжают получать `contextKey`.

Более подробный пример работы с контекстом пользователя можно увидеть в [демо-решениях](#demo-resheniq).
...
Более подробный пример использования SDK можно увидеть в [демо-решениях](#demo-resheniq).
...
| 1005       | Документ для пользователя доступен только для чтения                                                                | Пользователь не имеет прав на редактирование документа, изменение через протокол **update-provider** невозможно                                                                                             | `Документ для пользователя доступен только для чтения`                                            |
| 1006       | Ошибка обработки сообщения %message.name%: %пояснение%                                                              | Хост-окно не смогло обработать сообщение по своей вине: обработка не завершилась за отведённое время или произошла внутренняя ошибка. Данные сообщения корректны, запрос можно повторить                     | `Ошибка обработки сообщения UpdateRequest: Update task processing timeout`                        |
...
Одинаковый набор sandbox-флагов применяется ко всем трём типам iframe: главному iframe, виджетам и
кастомным модальным окнам. Единственное отличие — главный iframe дополнительно получает разрешение
`fullscreen` в атрибуте `allow`.
...
|--------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `allow-top-navigation`                     | Прямая навигация top-level страницы запрещена — разработчик не может программно перенаправить пользователя со страницы МоегоСклада. Для навигации по страницам МоегоСклада используйте протокол [navigation-service](#protokol-nawigacii) |
| `allow-top-navigation-by-user-activation`  | Запрещено даже по клику пользователя — iframe решения не должен перехватывать клики для перенаправления top-level страницы. Используйте [navigation-service](#protokol-nawigacii).                                                        |
...
`clipboard-read` и `clipboard-write` разрешают всем типам iframe работать с буфером обмена (копирование/вставка).
...
Кнопки начинают отображаться на странице только после перехода решения в статус **Activated**.

Кнопки можно встраивать как в карточку документа (сущности) так и в списки.
В случае работы с конкретным документом в контексте обработки нажатия придет идентификатор открытого объекта.
При работе со списками в контексте обработки придет массив выделенных объектов и их типы. 

В списках пользователю дается возможность выбрать до 100 объектов перед нажатием кнопки. 
...
Если кнопка не зависит от выбранных элементов, диалог подтверждения можно пропустить. См. атрибут `useSelected` в [buttons](#blok-buttons).

Для добавления кастомной кнопки в меню Решения необходимо заполнить [блок buttons](#blok-buttons) в дескрипторе и реализовать [эндпоинт обработчика нажатия на кнопку в vendorApi](#obrabotka-nazhatiq-na-kastomnuu-knopku).
...
Действия в сценариях позволяют администратору аккаунта настроить обработку дополнительных действий путем выбора из списка установленных решений 
(находящихся в статусе **Activated**). 
Настройка производится на странице [сценариев в МоемСкладе](https://online.moysklad.ru/app/#scripttemplate).

Пример настройки действия в сценариях:

![useful image](scenario-setup.png)
...
![useful image](scenario-audit.png)
