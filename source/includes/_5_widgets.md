...
В случае, если виджет использует протокол **open-feedback**, система не отображает 
 ...
 При сохранении страницы с виджетом, если виджет на экране редактирования сущности 
 использует протокол **save-handler**, он оповещается о факте сохранения объекта пользователем, 
 получая сообщение `Save`.
 
 Если виджет использует протокол **dirty-state**, то, при внесении изменений пользователем 
 в виджете, он отправляет хост-окну сообщение `SetDirty`. Если в виджете есть несохраненные изменения,
 и пользователь жмет кнопку "Сохранить" или закрывает окно с виджетом, 
 отображается подтверждение закрытия окна. Виджет может отправить хост-окну сообщение `ClearDirty`,
 после чего диалог подтвержения закрытия окна не будет появляться.
 
 Использование виджетом протоколов **open-feedback**, **save-handler**, **dirty-state** необходимо указать в [дескрипторе](#deskriptor-prilozheniq) 
  приложения.
...
нужно объявить поддержку опционального протокола **dirty-state**.
 ...
 После внесения пользователем изменений в виджете, он отправляет хост-окну
  сообщение `SetDirty`. Система учитывает, что в виджете есть несохраненные изменения.
  Далее, если пользователь нажимает кнопку “Закрыть“ (или другим способом пытается уйти с формы редактирования), 
