...
При использовании атрибута `data-manual-zoom`:

* при открытии инструкции в каталоге МоегоСклада изображение будет показано в модальном окне;
* при открытии инструкции по прямой ссылке изображение откроется в новой вкладке.
...
* для видео не поддерживаются специальные режимы отображения: атрибут `data-manual-zoom` не работает, а полноэкранный просмотр через `allowfullscreen` недоступен;
