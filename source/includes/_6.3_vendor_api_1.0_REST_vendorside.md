...
+ **selected** `Object[]` — список сущностей или документов в списке, для которых кнопка была нажата. Не заполняется для точек встраивания `*.create, *.edit`, а также для `*.list` с аттрибутом у кнопки `useSelected=false`. Поля:
