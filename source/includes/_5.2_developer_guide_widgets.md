...
Кастомные кнопки позволяют разработчику встраивать одну или несколько кнопок в меню Решения на странице документа МоегоСклада.
...
Для добавления кастомной кнопки необходимо заполнить [блок buttons](#blok-buttons) в дескрипторе приложения и реализовать [эндпоинт обработчика нажатия на кнопку в vendorApi](#obrabotka-nazhatiq-na-kastomnuu-knopku).

Пример дескриптора решения с кнопками на странице Заказа покупателя расположен в правой части экрана.

Визуальная часть кнопки — элемент меню, которое появляется при нажатии на кнопку Решения в документе.

Пример меню, которое раскрывается при нажатии на кнопку Решения:
