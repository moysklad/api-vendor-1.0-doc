...
| [v2](https://apps-api.moysklad.ru/xml/ns/appstore/app/v2/application-v2.xsd) |       ✅        | [Виджеты](#vidzhety) в документах и сущностях. Кастомные модальные окна. Гибкие права решений. Дополнительные и сервисные протоколы. | vendorApi, access(с permissions), iframe, iframes, widgets, popups | Серверные                   |
...
* **Специальные** — права для работы с вебхуками и дополнительными полями (подробнее: [вебхуки](#rabota-s-webhukami), [доп. поля](#rabota-s-dopolnitel-nymi-polqmi)).
...
- Чтобы задать дополнительные протоколы, которые будут использоваться в iframe, используйте тег `uses` (опциональный). В настоящий момент поддерживается только для типа `main`. Список поддерживаемых значений совпадает со списком [блока **uses** для виджетов](#blok-serwisnyh-protokolow-uses).
- Атрибут **useContextKey** — опциональный, тип `boolean`, значение по умолчанию `true`. Он определяет, будет ли
  ...
  [протоколу user-context](#protokol-user-context). Атрибут доступен также в [блоке iframe](#blok-iframe),
  [блоке widgets](#blok-widgets) и [блоке popups](#blok-popups).

Подробнее про [окна решений](#okna-iframes). 
...
Список поддерживаемых значений совпадает со списком [блока **uses** для виджетов](#blok-serwisnyh-protokolow-uses).

Данный блок считается устаревшим. Вместо него следует использовать [блок **iframes**](#blok-iframes).
...
Виджет можно скрыть, установив `height/fixed = 0px`. Скрытые виджеты не отображаются на страницах МоегоСклада, но могут использовать все допустимые [протоколы](#protokoly-widzhetow).
...
Подробнее о дополнительных протоколах читайте в разделе [Виджеты](#vidzhety).
...
* **user-context** позволяет виджетам, главному окну и модальным окнам запросить у хост-окна одноразовый токен
  ...
  разделе [Протокол user-context](#protokol-user-context).
