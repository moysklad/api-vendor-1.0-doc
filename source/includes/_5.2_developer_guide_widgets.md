...
При реализации окон решения следует учитывать что если в iframe будет какая-либо пользовательская функциональность кроме настроек, предусмотрите доступ к этой функциональности для пользователей с разными правами доступа, не только для администратора.
Для этого используйте [контекст пользователя](#kontext-pol-zowatelq).
