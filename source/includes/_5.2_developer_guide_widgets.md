...
Особенности:
- В документе **Заказ кодов маркировки** после отправки в **Честный Знак** (через кнопку `Заказ кодов`):
  - хоть пользовать и виджет могут редактировать данные в интерфейсе, их сохранение больше недоступно, следует учитывать это поведении при использовании протоколов;
  - на изменение в интерфейсе статуса документа или позиций в СУЗ будут приходить сообщения `Change` без последующего `Save`.
