...
Чтобы решение могло использовать кастомные модальные окна, добавьте в [дескриптор решения](#blok-popups) блок:
...
* в настоящее время не поддерживаются сервисы хост-окна [селектор группы товаров](#selektor-gruppy-towarow) и [стандартные диалоги](#standartnye-dialogi).
