...
Примеры JS-кода можно посмотреть в [демо-приложении](https://github.com/moysklad/php-dummyapp-marketplace-1.0/blob/master/src/php/widgets/widget.html.php) на GitHub. Примеры демонстрируют:

- как получить сообщение `Open` с идентификатором документа `objectId`, в котором открыт виджет;
- как залогировать в консоль входящие и исходящие сообщения (от виджета и к виджету).
