...
1. Создайте версию для редактирования (как описано выше). Для начальной разработки также можно создать отдельное решение с собственным псевдонимом (например, `myapp_dev`), чтобы не затрагивать опубликованное решение.
2. В дескрипторе версии (или отдельного решения) укажите URL вашего тестового сервера в теге [vendorApi/endpointBase](#blok-vendorapi) (например, `https://my-test-server.com/app`).
...
Если изменения затрагивают только контентную часть решения (описание, скриншоты, инструкцию, названия и описания тарифов) и не касаются дескриптора или серверной логики, полный цикл с переключением URL не нужен. Достаточно создать версию, внести контентные изменения и отправить её на модерацию.
