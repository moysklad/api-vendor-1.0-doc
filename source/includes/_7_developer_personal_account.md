...
- **Черновик (Draft)** — статус черновика, предназначен для разработки и тестирования. Решение не отображается на витрине у пользователей МоегоСклада.
- **Готово, отправлено на модерацию (Ready)** — статус готовности. Решение готово к публикации со стороны разработчика. Сотрудники МоегоСклада проверят решение перед публикацией. Решение не отображается на витрине у пользователей МоегоСклада.
- **Опубликовано (Published)** — решение опубликовано на витрине решений и доступно к установке пользователями МоегоСклада.
- **Снято с публикации (Hidden)** — публикация решения приостановлена. Решение исчезает с витрины, но у текущих пользователей решения сохраняется возможность работать с ним. 
- **Версия опубликована (Merged)** — версия решения опубликована на витрине решений. Данный статус является внутренним, и не отображается в списке решений и на витрине. 
...
2. Сотрудник МоегоСклада проверяет, затем публикует решение на витрине. Статус решения меняется на **Published**. Решение становится доступно пользователю МоегоСклада для установки.
...
- **Прямая ссылка на решение**. Публичная ссылка для прямого доступа к решению на витрине решений МС.
...
- **Название тарифа**. Должно быть кратким и давать представление о сути тарифного плана. Примеры: Лайт, Мидл, Профи и т.д. Для решений с одним тарифом не выводится на витрине.
...
Перед публикацией на витрине МоегоСклада новое решение должно пройти модерацию.
...
После создания версии решения работать с ней можно как с обычным черновиком: изменять параметры, отлаживать на витрине и т.д.
...
5. Чтобы отладить решение при возобновлении работы, нажмите на кнопку **Активировать** в карточке решения на витрине МоегоСклада.
...
Отладить неопубликованное решение можно в аккаунте разработчика. Неопубликованные решения разработчика находятся на витрине решений в блоке **Решения в разработке**. Неопубликованные решения видит только разработчик, для пользователей они не видны. 
...
При удалении решения на витрине МоегоСклада пользователю предлагается оставить отзыв о решении:
...
Также прямую ссылку можно получить в витрине решений из адресной строки браузера, если открыть карточку решения на витрине.

При переходе по короткой ссылке открывается витрина с карточкой решения. Если пользователь авторизован, витрина с карточкой открывается сразу. Если нет, для просмотра необходимо авторизаваться.

Если решение было удалено или недоступно по прямой ссылке, при переходе по ссылке открывается витрина и появляется сообщение о том, что решение не найдено.
 
Если решение снято с публикации, но уже установлено некоторыми пользователями, оно будет отображаться для этих пользователей по ссылке как на витрине. Для остальных пользователей решение будет недоступно.
...
Инструкция отображается на странице решения на витрине МоегоСклада, а также в главном iframe решения.
