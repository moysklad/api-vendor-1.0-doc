...
Об ограничениях и разрешениях iframe читайте в разделе [Политика безопасности iframe](#politika-bezopasnosti-iframe).
...
МойСклад загружает весь вендорский контент (главный iframe, виджеты и кастомные модальные окна) внутри
sandboxed iframe с определёнными HTML-атрибутами `sandbox` и `allow`. Эти атрибуты устанавливаются
платформой МоегоСклада и не могут быть изменены вендором.
...
Атрибут `sandbox` управляет ограничениями встроенного контента. Следующие флаги **включены** для всех iframe вендорских решений:
...
Следующие флаги **не включены** в sandbox-политику. Соответствующие возможности заблокированы для вендорского кода:

| Флаг                                       | Почему не включён                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
|--------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `allow-top-navigation`                     | Прямая навигация top-level страницы запрещена — вендор не может программно перенаправить пользователя со страницы МоегоСклада. Для навигации по страницам МоегоСклада используйте протокол [navigation-service](#protokol-nawigacii), который обеспечивает контролируемую навигацию через валидированные `NavigateRequest`-сообщения. Путь должен начинаться с `#`, навигация возможна с `target: "self"` (текущая вкладка) или `"blank"` (новая вкладка). Для использования укажите `<navigation-service/>` в теге `<uses>` дескриптора. |
| `allow-top-navigation-by-user-activation`  | Запрещено даже по клику пользователя — вендорский iframe не должен перехватывать клики для перенаправления top-level страницы. Используйте [navigation-service](#protokol-nawigacii).                                                                                                                                                                                                                                                                                                                                                     |
| `allow-top-navigation-to-custom-protocols` | Запрещена навигация на кастомные протоколы (`tel:`, `mailto:` и др.). Вендор не должен инициировать открытие сторонних приложений из контекста МоегоСклада без контроля платформы.                                                                                                                                                                                                                                                                                                                                                        |
| `allow-popups-to-escape-sandbox`           | Попапы, открытые из iframe через `window.open()`, наследуют те же sandbox-ограничения. Без этого ограничения вендор мог бы через попап обойти sandbox-политику и получить привилегии, не предусмотренные платформой.                                                                                                                                                                                                                                                                                                                      |
| `allow-orientation-lock`                   | МойСклад — десктопное веб-приложение, блокировка ориентации экрана не применима в контексте сервиса.                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `allow-pointer-lock`                       | Захват курсора мыши (Pointer Lock API, используется в играх и 3D-приложениях) не требуется для бизнес-логики вендорских решений. Кроме того, может ухудшить UX, неожиданно заблокировав курсор пользователя.                                                                                                                                                                                                                                                                                                                              |
| `allow-presentation`                       | Presentation API (трансляция на внешние дисплеи) не применим в контексте работы с МоимСкладом.                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `allow-storage-access-by-user-activation`  | Экспериментальная возможность браузера для доступа к unpartitioned cookies. Не требуется: вендор работает с cookies и storage своего домена через `allow-same-origin`, доступ к cookies МоегоСклада не предусмотрен.                                                                                                                                                                                                                                                                                                                      |

#### Атрибут allow (Permissions Policy)

Атрибут `allow` управляет доступом к API браузера (Permissions Policy) внутри iframe:
...
- Формы, скрипты и API-вызовы работают без ограничений.
- Доступ к буферу обмена (clipboard) разрешён.
- Можно открывать попапы через `window.open()`, но они наследуют sandbox-ограничения.
- Можно использовать `alert()`, `confirm()`, `prompt()`. Однако для более гибкого взаимодействия с пользователем рекомендуется использовать [кастомные модальные окна](#kastomnye-modal-nye-okna).
- Можно инициировать скачивание файлов.
- **Нельзя** напрямую перенаправлять родительскую страницу (`window.top.location`, `target="_top"`). Для навигации по страницам МоегоСклада используйте протокол [navigation-service](#protokol-nawigacii), указав `<navigation-service/>` в теге `<uses>` дескриптора.
- Fullscreen доступен **только** в главном iframe, но не в виджетах и модальных окнах.
- Cookies и localStorage доступны для своего домена (благодаря `allow-same-origin`), но не для домена МоегоСклада.
